"""
Stalkerware IoC Web Scraper
============================
Source: https://github.com/AssoEchap/stalkerware-indicators

copy env.template to .env and fill in your database credentials before running.
"""

import os
import sys
import csv
import io
import requests
import yaml
from datetime import datetime, date
from dotenv import load_dotenv

load_dotenv()

# ─── Try importing pymysql ─────────────────────────────────────────────────
try:
    import pymysql
    import pymysql.cursors
except ImportError:
    print("[ERROR] pymysql not installed. Run: pip install pymysql")
    sys.exit(1)

# ─── Database Config ───────────────────────────────────────────────────────
DB_CONFIG = {
    "host":     os.getenv("DB_HOST", "localhost"),
    "port":     int(os.getenv("DB_PORT", 3306)),
    "user":     os.getenv("DB_USER", "root"),
    "password": os.getenv("DB_PASSWORD", ""),
    "database": os.getenv("DB_NAME", "stalkerware_iocs"),
    "charset":  "utf8mb4",
    "cursorclass": pymysql.cursors.DictCursor,
}

# ─── Source URLs (ECHAP GitHub via jsDelivr CDN mirror) ────────────────────
SOURCES = {
    "ioc.yaml": [
        "https://raw.githubusercontent.com/AssoEchap/stalkerware-indicators/master/ioc.yaml",
        "https://cdn.jsdelivr.net/gh/AssoEchap/stalkerware-indicators@master/ioc.yaml",
    ],
    "watchware.yaml": [
        "https://raw.githubusercontent.com/AssoEchap/stalkerware-indicators/master/watchware.yaml",
        "https://cdn.jsdelivr.net/gh/AssoEchap/stalkerware-indicators@master/watchware.yaml",
    ],
    "samples.csv": [
        "https://raw.githubusercontent.com/AssoEchap/stalkerware-indicators/master/samples.csv",
        "https://cdn.jsdelivr.net/gh/AssoEchap/stalkerware-indicators@master/samples.csv",
    ],
}

SOURCE_REFERENCE = "ECHAP-AssoEchap-GitHub"

# ─── IoC type mapping: yaml key → your ioc_type column value ───────────────
IOC_TYPE_MAP = {
    "domains":      "domain",
    "packages":     "package_name",
    "certificates": "certificate",
    "sha256":       "file_hash",
    "filenames":    "filename",
    "processes":    "process_name",
    "emails":       "email",
    "ips":          "ip_address",
}

# ─── Severity mapping based on IoC type ────────────────────────────────────
SEVERITY_MAP = {
    "certificate":  "High",
    "package_name": "High",
    "file_hash":    "High",
    "ip_address":   "High",
    "domain":       "Medium",
    "process_name": "Medium",
    "filename":     "Medium",
    "email":        "Low",
}

# ─── Relationship type mapping ──────────────────────────────────────────────
RELATIONSHIP_MAP = {
    "domain":       "communicates_with",
    "ip_address":   "connects_to",
    "package_name": "uses",
    "file_hash":    "drops",
    "certificate":  "signed_with",
    "filename":     "drops",
    "process_name": "runs",
    "email":        "registered_with",
}

# ─── Fetch Helper ───────────────────────────────────────────────────────────

def fetch_url(filename):
    """Try each URL for a file, return raw text content."""
    urls = SOURCES[filename]
    headers = {"User-Agent": "Mozilla/5.0 (stalkerware-ioc-scraper)"}

    for url in urls:
        try:
            print(f"    Trying {url} ...")
            resp = requests.get(url, headers=headers, timeout=15)
            resp.raise_for_status()
            print(f"    OK ({len(resp.content)} bytes)")
            return resp.text
        except Exception as e:
            print(f"    Failed: {e}")

    raise RuntimeError(f"Could not fetch {filename} from any source.")

# ─── DB Helpers ─────────────────────────────────────────────────────────────

def get_connection():
    return pymysql.connect(**DB_CONFIG)


def get_or_insert_app(cur, app_name, platform, release_date):
    """
    Insert stalkerware app if it doesn't exist.
    Returns app_id.
    """
    cur.execute(
        "SELECT app_id FROM stalkerware_apps WHERE app_name = %s",
        (app_name[:40],)  # varchar(40) limit
    )
    row = cur.fetchone()
    if row:
        return row["app_id"]

    cur.execute(
        """INSERT INTO stalkerware_apps (app_name, platform, release_date)
           VALUES (%s, %s, %s)""",
        (app_name[:40], platform[:40], release_date)
    )
    return cur.lastrowid


def get_or_insert_ioc(cur, ioc_type, ioc_value, severity_level, date_identified):
    """
    Insert IoC if it doesn't exist (match on type + value).
    Returns ioc_id.
    """
    cur.execute(
        "SELECT ioc_id FROM iocs WHERE ioc_type = %s AND ioc_value = %s",
        (ioc_type[:40], ioc_value[:255])
    )
    row = cur.fetchone()
    if row:
        return row["ioc_id"]

    cur.execute(
        """INSERT INTO iocs (ioc_type, ioc_value, severity_level, date_identified)
           VALUES (%s, %s, %s, %s)""",
        (ioc_type[:40], ioc_value[:255], severity_level[:40], date_identified)
    )
    return cur.lastrowid


def link_app_ioc(cur, app_id, ioc_id, relationship_type, source_reference):
    """
    Link app to IoC in app_ioc table.
    Skip if already linked.
    """
    cur.execute(
        "SELECT app_ioc_id FROM app_ioc WHERE app_id = %s AND ioc_id = %s",
        (app_id, ioc_id)
    )
    if cur.fetchone():
        return  # already linked

    cur.execute(
        """INSERT INTO app_ioc (relationship_type, source_reference, app_id, ioc_id)
           VALUES (%s, %s, %s, %s)""",
        (relationship_type[:40], source_reference[:40], app_id, ioc_id)
    )


# ─── Ingest YAML (ioc.yaml / watchware.yaml) ────────────────────────────────

def ingest_yaml(cur, filename, default_platform="Android"):
    """
    Parse YAML file and insert into stalkerware_apps, iocs, app_ioc.

    Expected YAML structure per entry:
      - name: FlexiSpy
        domains:
          - flexispy.com
        packages:
          - com.flexispy.android
        certificates:
          - abc123...
        website: https://flexispy.com
    """
    print(f"\n  Fetching {filename} ...")
    raw = fetch_url(filename)
    entries = yaml.safe_load(raw)

    if not entries or not isinstance(entries, list):
        print(f"  [WARN] {filename} is empty or unexpected format.")
        return {"apps": 0, "iocs": 0, "links": 0}

    stats = {"apps": 0, "iocs": 0, "links": 0}
    today = date.today()

    for entry in entries:
        if not isinstance(entry, dict):
            continue

        name = str(entry.get("name", "")).strip()
        if not name:
            continue

        # Infer platform from package names (Android) or default
        has_packages = bool(entry.get("packages"))
        platform = "Android" if has_packages else default_platform

        app_id = get_or_insert_app(cur, name, platform, today)
        stats["apps"] += 1

        # Loop through all IoC field types
        for yaml_key, ioc_type in IOC_TYPE_MAP.items():
            values = entry.get(yaml_key, [])
            if not values:
                continue
            if isinstance(values, str):
                values = [values]

            for value in values:
                value = str(value).strip()
                if not value:
                    continue

                severity     = SEVERITY_MAP.get(ioc_type, "Medium")
                relationship = RELATIONSHIP_MAP.get(ioc_type, "related_to")

                ioc_id = get_or_insert_ioc(
                    cur, ioc_type, value, severity, today
                )
                stats["iocs"] += 1

                link_app_ioc(
                    cur, app_id, ioc_id, relationship, SOURCE_REFERENCE
                )
                stats["links"] += 1

    return stats


# ─── Ingest samples.csv (file hashes) ───────────────────────────────────────

def ingest_samples_csv(cur):
    """
    samples.csv columns: sha256, app_name, [package, certificate, version]
    Inserts SHA256 hashes as file_hash IoCs linked to their app.
    Only inserts if the app already exists from YAML ingestion.
    """
    print(f"\n  Fetching samples.csv ...")
    raw = fetch_url("samples.csv")

    reader = csv.DictReader(io.StringIO(raw))
    stats  = {"iocs": 0, "skipped": 0}
    today  = date.today()

    for row in reader:
        sha256   = row.get("sha256", "").strip()
        app_name = row.get("app_name", "").strip()

        if not sha256 or not app_name:
            stats["skipped"] += 1
            continue

        # Only attach hash if the app was already ingested from YAML
        cur.execute(
            "SELECT app_id FROM stalkerware_apps WHERE app_name = %s",
            (app_name[:40],)
        )
        app_row = cur.fetchone()
        if not app_row:
            stats["skipped"] += 1
            continue

        app_id = app_row["app_id"]
        ioc_id = get_or_insert_ioc(cur, "file_hash", sha256[:40], "High", today)
        link_app_ioc(cur, app_id, ioc_id, "drops", SOURCE_REFERENCE)
        stats["iocs"] += 1

    return stats


# ─── Main ────────────────────────────────────────────────────────────────────

def main():
    print("=" * 60)
    print("  Stalkerware IoC Scraper — ECHAP GitHub Source")
    print("=" * 60)

    # Test DB connection first
    print("\n[0] Connecting to database ...")
    try:
        conn = get_connection()
        print(f"    Connected to '{DB_CONFIG['database']}' on {DB_CONFIG['host']}")
    except Exception as e:
        print(f"\n[ERROR] Could not connect to database: {e}")
        print("\nCheck your .env file has the correct credentials:")
        print("  DB_HOST, DB_PORT, DB_USER, DB_PASSWORD, DB_NAME")
        sys.exit(1)

    cur = conn.cursor()

    try:
        # Step 1: Ingest stalkerware (ioc.yaml)
        print("\n[1/3] Ingesting stalkerware — ioc.yaml ...")
        stats_ioc = ingest_yaml(cur, "ioc.yaml", default_platform="Android")
        print(f"      Apps: {stats_ioc['apps']} | IoCs: {stats_ioc['iocs']} | Links: {stats_ioc['links']}")

        # Step 2: Ingest watchware (watchware.yaml)
        print("\n[2/3] Ingesting watchware — watchware.yaml ...")
        stats_watch = ingest_yaml(cur, "watchware.yaml", default_platform="Android")
        print(f"      Apps: {stats_watch['apps']} | IoCs: {stats_watch['iocs']} | Links: {stats_watch['links']}")

        # Step 3: Ingest file hashes (samples.csv)
        print("\n[3/3] Ingesting file hashes — samples.csv ...")
        stats_samples = ingest_samples_csv(cur)
        print(f"      Hashes inserted: {stats_samples['iocs']} | Skipped: {stats_samples['skipped']}")

        conn.commit()

        # Summary
        total_apps = stats_ioc["apps"] + stats_watch["apps"]
        total_iocs = stats_ioc["iocs"] + stats_watch["iocs"] + stats_samples["iocs"]
        total_links = stats_ioc["links"] + stats_watch["links"] + stats_samples["iocs"]

        print("\n" + "=" * 60)
        print("  Scrape Complete!")
        print(f"  Total apps inserted : {total_apps}")
        print(f"  Total IoCs inserted : {total_iocs}")
        print(f"  Total links created : {total_links}")
        print("=" * 60)

    except Exception as e:
        conn.rollback()
        print(f"\n[ERROR] {e}")
        import traceback
        traceback.print_exc()
        sys.exit(1)

    finally:
        cur.close()
        conn.close()


if __name__ == "__main__":
    main()
